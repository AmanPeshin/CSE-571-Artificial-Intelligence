...
def axiom_generator_percept_sentence(t, tvec):
    ...
    temp = []
    time = str(t)
    for x in range(len(tvec)):
        if x == 0:
            if not tvec[x]:
                temp.append('~' + 'Stench' + time)
            else:
                temp.append('Stench' + time)
        if x == 1:
            if not tvec[x]:
                temp.append('~' + 'Breeze' + time)
            else:
                temp.append('Breeze' + time)
        if x == 2:
            if not tvec[x]:
                temp.append('~' + 'Glitter' + time)
            else:
                temp.append('Glitter' + time)
        if x == 3:
            if not tvec[x]:
                temp.append('~' + 'Bump' + time)
            else:
                temp.append('Bump' + time)
        if x == 4:
            if not tvec[x]:
                temp.append('~' + 'Scream' + time)
            else:
                temp.append('Scream' + time)
    axiom_str = ' & '.join(temp)
    return axiom_str
...
def axiom_generator_initial_location_assertions(x, y):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    filler = [pit_str(x, y), wumpus_str(x, y)]
    axiom_str = ''
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    y_loc = [y, (y - 1), y, (y + 1)]
    length_loc = 4
    for i in range(0, length_loc):
        a = x_loc[i]
        b = y_loc[i]
        if xmin <= a <= xmax and ymin <= b <= ymax:
            pit_at_x_y = pit_str(a, b)
            pit_loc.append(pit_at_x_y)
    temp_loc = 'P' + str(x) + '_' + str(y)
    pit_loc.append(temp_loc)
    breeze_at_x_y = breeze_str(x, y)
    pit_loc_joined = ' | '.join(pit_loc)
    axiom_str += '{0} <=> ({1})'.format(
        breeze_at_x_y, pit_loc_joined)
    return axiom_str
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    for i in range(0, length_loc):
        ...
        b = y_loc[i]
        if xmin <= a <= xmax and ymin <= b <= ymax:
            wumpus_at_x_y = wumpus_str(a, b)
            wump.append(wumpus_at_x_y)
    temp_loc = 'W' + str(x) + '_' + str(y)
    wump.append(temp_loc)
    stench_at_x_y = stench_str(x, y)
    wumpus_loc_joined = ' | '.join(wump)
    axiom_str += '{0} <=> ({1})'.format(
        stench_at_x_y, wumpus_loc_joined)
    return axiom_str
...
def axiom_generator_at_least_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    for x in range(xmin, xmax + 1):
        for y in range(ymin, ymax + 1):
            wumpus_present_x_y = wumpus_str(x, y)
            wump.append(wumpus_present_x_y)
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    for env in envs:
        ...
        not_in_env = []
        for thisRoom in envs:
            if thisRoom != env:
                not_in_env.append(thisRoom)

        no_wumpus_dict = []
        for w in not_in_env:
            wumpus_at_x_y = wumpus_str(w[0], w[1])
            no_wumpus_dict.append('~' + wumpus_at_x_y)

        wumpus_at_env_x_y = wumpus_str(env[0], env[1])
        wumpus_loc_joins = ' & '.join(no_wumpus_dict)
        axioms.append('({0} >> ({1}))'.format(
            wumpus_at_env_x_y, wumpus_loc_joins))
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t=0):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    currentenv = ('~L' + str(xi) + '_' + str(yi) + '_' + str(t))
    not_env = []
    for xVal in range(xmin, xmax + 1):
        for yVal in range(ymin, ymax + 1):
            at_loc_x_y_at_t = state_loc_str(xVal, yVal, t)
            not_env.append('~' + at_loc_x_y_at_t)

    not_env.remove(currentenv)
    temp1 = 'L{0}_{1}_{2}'.format(
        xi, yi, t)
    temp2 = '{0}'.format(' & '.join(not_env))
    return temp1 + " & " + temp2
...
def axiom_generator_only_one_heading(heading='north', t=0):
    ...
    temp = []
    for i in range(0, 4):
        if heading.lower() == directions[i].lower():
            axiom_str += 'Heading{0}{1}'.format(
                directions[i], str(t))
        else:
            temp.append('~Heading' + directions[i] + str(t))
    axiom_str += ' & '
    axiom_str += '{0}'.format('&'.join(temp))
    return axiom_str
...
def axiom_generator_have_arrow_and_wumpus_alive(t=0):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    filler = [state_have_arrow_str(t), state_wumpus_alive_str(t)]
    axiom_str = '{0} & {1}'.format(
        filler[0], filler[1])
    return axiom_str
...
def axiom_generator_location_OK(x, y, t):
    ...
    filler = [state_OK_str(x, y, t), pit_str(x, y), state_wumpus_alive_str(t), wumpus_str(x, y)]

    axiom_str = '{0} <=> (~{1} & ({3} >> ~{2}))'.format(
...
def axiom_generator_breeze_percept_and_location_property(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    filler = [state_loc_str(x, y, t), percept_breeze_str(t), breeze_str(x, y)]
...
def axiom_generator_stench_percept_and_location_property(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    filler = [state_loc_str(x, y, t), percept_stench_str(t), stench_str(x, y)]
    axiom_str = '{0} >> ({1} % {2})'.format(
        filler[0], filler[1], filler[2])
    return axiom_str
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    loc_x_y_at_time_t = state_loc_str(x, y, t)
    bump_percieved_at_time_t_plus_1 = percept_bump_str(t + 1)
    forward_action_at_time_t = action_forward_str(t)
    grab_action_done_at_time_t = action_grab_str(t)
    shoot_action_at_time_t = action_shoot_str(t)
    action_left_at_time_t = action_turn_left_str(t)
    action_right_at_time_t = action_turn_right_str(t)
    temp = ['({0} & (~{1} | {2} | {3} | {4} | {5} | {6}))' \
                .format(loc_x_y_at_time_t, forward_action_at_time_t, bump_percieved_at_time_t_plus_1,
                        grab_action_done_at_time_t, shoot_action_at_time_t,
                        action_left_at_time_t, action_right_at_time_t)]
    x_locs = [(x - 1), x, (x + 1), x]
    y_locs = [y, (y - 1), y, (y + 1)]
    ...
    for i in range(0, iter_len):
        ...
        if xmin <= a <= xmax and ymin <= b <= ymax:
            if m == 'East':
                loc_x_y_at_time_t = state_loc_str(x - 1, y, t)
                heading_east_at_time_t = state_heading_east_str(t)
                forward_action_at_time_t = action_forward_str(t)
                temp.append('({0} & ({1} & {2}))'.format(
                    loc_x_y_at_time_t, heading_east_at_time_t, forward_action_at_time_t))
            if m == 'North':
                loc_x_y_at_time_t = state_loc_str(x, y - 1, t)
                heading_north_at_time_t = state_heading_north_str(t)
                forward_action_at_time_t = action_forward_str(t)
                temp.append('({0} & ({1} & {2}))'.format(
                    loc_x_y_at_time_t, heading_north_at_time_t, forward_action_at_time_t))
            if m == 'South':
                loc_x_y_at_time_t = state_loc_str(x, y + 1, t)
                heading_south_at_time_t = state_heading_south_str(t)
                forward_action_at_time_t = action_forward_str(t)
                temp.append('({0} & ({1} & {2}))'.format(
                    loc_x_y_at_time_t, heading_south_at_time_t, forward_action_at_time_t))
            if m == 'West':
                loc_x_y_at_time_t = state_loc_str(x + 1, y, t)
                heading_west_at_time_t = state_heading_west_str(t)
                forward_action_at_time_t = action_forward_str(t)
                temp.append('({0} & ({1} & {2}))'.format(
                    loc_x_y_at_time_t, heading_west_at_time_t, forward_action_at_time_t))
    axiom_str = '{0} <=> ({1})'.format(state_loc_str(x, y, t + 1), ' | '.join(temp))
    return axiom_str
...
def axiom_generator_have_arrow_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    has_an_arrow_at_time_t = state_have_arrow_str(t)
    can_shoot_an_arrow_at_time_t = action_shoot_str(t)
    has_an_arrow_at_time_t_plus_1 = state_have_arrow_str(t + 1)
    filler = [has_an_arrow_at_time_t_plus_1, has_an_arrow_at_time_t, can_shoot_an_arrow_at_time_t]
    axiom_str = '{0} <=> ({1} & ~{2})'.format(
        filler[0], filler[1], filler[2])
    return axiom_str
...
def axiom_generator_wumpus_alive_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    scream_at_time_t = percept_scream_str(t + 1)
    wumpus_alive_at_time_t = state_wumpus_alive_str(t)
    wumpus_alive_at_time_t_plus_1 = state_wumpus_alive_str(t + 1)
    filler = [wumpus_alive_at_time_t_plus_1, wumpus_alive_at_time_t, scream_at_time_t]
    axiom_str = '{0} <=> ({1} & ~{2})'.format(
        filler[0], filler[1], filler[2])
    return axiom_str
...
def axiom_generator_heading_north_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    turn_east_at_time_t = state_heading_east_str(t)
    turn_west_at_time_t = state_heading_west_str(t)
    heading_north_at_time_t = state_heading_north_str(t)
    turn_left_at_time_t = action_turn_left_str(t)
    turn_north_at_time_t = action_turn_right_str(t)
    heading_north_at_time_t_plus_1 = state_heading_north_str(t + 1)

    filler = [heading_north_at_time_t_plus_1, heading_north_at_time_t, turn_left_at_time_t, turn_north_at_time_t, turn_east_at_time_t, turn_west_at_time_t]

    axiom_str = filler[0] + ' <=> ((' + filler[1] + ' & ~' + filler[2] + ' & ~' + filler[3] +\
                ') | (' + filler[4] + ' & ' + filler[2] + ') | (' + filler[5] + ' & ' + filler[3] + '))'
...
def axiom_generator_heading_east_ssa(t):
    ...
    heading_east_at_time_t = state_heading_east_str(t)
    turn_left_at_time_t = action_turn_left_str(t)
    turn_right_at_time_t = action_turn_right_str(t)
    heading_east_at_time_t_plus_1 = state_heading_east_str(t+1)

    filler = [heading_east_at_time_t_plus_1, heading_east_at_time_t, turn_left_at_time_t, turn_right_at_time_t, heading_south_at_time_t, heading_north_at_time_t]

    axiom_str = filler[0] + ' <=> ((' + filler[1] + ' & ~' + filler[2] +\
                ' & ~' + filler[3] + ') | (' + filler[4] + ' & ' +\
                filler[2] + ') | (' + filler[5] + ' & ' + filler[3] + '))'
    return axiom_str
...
def axiom_generator_heading_south_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    turning_west_at_time_t = state_heading_west_str(t)
    turn_east_at_time_t = state_heading_east_str(t)
    heading_south_at_time_t = state_heading_south_str(t)
    turn_left_at_time_t = action_turn_left_str(t)
    heading_south_at_time_t_plus_1 = state_heading_south_str(t+1)
    turn_right_at_time_t = action_turn_right_str(t)

    filler = [heading_south_at_time_t_plus_1, heading_south_at_time_t, turn_left_at_time_t, turn_right_at_time_t, turning_west_at_time_t, turn_east_at_time_t]

    axiom_str = filler[0] + ' <=> ((' + filler[1] + ' & ~' + filler[2] +\
                ' & ~' + filler[3] + ') | (' + filler[4] + ' & ' + filler[2] +\
                ') | (' + filler[5] + ' & ' + filler[3] + '))'
    return axiom_str
...
def axiom_generator_heading_west_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    turn_north_at_time_t = state_heading_north_str(t)
    turn_south_at_time_t = state_heading_south_str(t)
    heading_west_at_time_t = state_heading_west_str(t)
    turn_left_at_time_t = action_turn_left_str(t)
    heading_west_at_time_t_plus_1 = state_heading_west_str(t+1)
    turn_right_at_time_t = action_turn_right_str(t)

    filler = [heading_west_at_time_t_plus_1, heading_west_at_time_t, turn_left_at_time_t, turn_right_at_time_t, turn_north_at_time_t, turn_south_at_time_t]
...
def axiom_generator_heading_only_north(t):
    ...
    heading_east_at_time_t = state_heading_east_str(t)
    heading_north_at_time_t = state_heading_north_str(t)
    filler = [heading_north_at_time_t, heading_south_at_time_t, heading_east_at_time_t, heading_west_at_time_t]
...
def axiom_generator_heading_only_east(t):
    ...
    heading_east_at_time_t = state_heading_east_str(t)
    heading_north_at_time_t = state_heading_north_str(t)
    filler = [heading_east_at_time_t, heading_south_at_time_t, heading_north_at_time_t, heading_west_at_time_t]
    axiom_str = '{0} <=> (~{1} & ~{2} & ~{3})'.format(
        filler[0], filler[1], filler[2], filler[3])
    return axiom_str
...
def axiom_generator_heading_only_south(t):
    ...
    heading_east_at_time_t = state_heading_east_str(t)
    heading_north_at_time_t = state_heading_north_str(t)
    filler = [heading_south_at_time_t, heading_north_at_time_t, heading_east_at_time_t, heading_west_at_time_t]
    axiom_str = '{0} <=> (~{1} & ~{2} & ~{3})'.format(
        filler[0], filler[1], filler[2], filler[3])
    return axiom_str
...
def axiom_generator_heading_only_west(t):
    ...
    heading_east_at_time_t = state_heading_east_str(t)
    heading_north_at_time_t = state_heading_north_str(t)
    filler = [heading_west_at_time_t, heading_south_at_time_t, heading_east_at_time_t, heading_north_at_time_t]
    axiom_str = '{0} <=> (~{1} & ~{2} & ~{3})'.format(
        filler[0], filler[1], filler[2], filler[3])
    return axiom_str
...
def axiom_generator_only_one_action_axioms(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    actions = ['Shoot', 'Climb', 'Forward', 'Grab', 'TurnRight', 'Wait', 'TurnLeft']
    axioms = []
    iter_len = 7
    for i in range(0, iter_len):
        temp = []
        for j in range(0, iter_len):
            if i != j:
                temp.append('~' + actions[j] + str(t))
        executed_action = actions[i]
        axioms.append('({0}{1} <=> ({2}))'.format(executed_action, str(t), ' & '.join(temp)))
